...
function GroupCard({group, className=''}) {
    ...
    let {id, wall_id, name, screen_name, info, is_closed, toxicity, avatar, is_saved} = group;
    ...
    async function changeSavedStatus() {
        let data = null
        if(isSaved)
            data = await $authMainHost.delete('api/toxicity/user/saved-records/group/' + id)
        else
            data = await $authMainHost.post('api/toxicity/user/saved-records', {object_id: id, object_type: 'group'})
